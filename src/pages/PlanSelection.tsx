...
const PlanSelection = () => {
  ...
  const handlePlanSelect = (planName: string, isFree: boolean = false) => {
    if (isFree) {
      navigate('/cadastro?plan=essencial');
    } else {
      // For paid plans, offer trial option
      navigate(`/cadastro?plan=${planName.toLowerCase()}&trial=true`);
    }
  };

  const handleDirectPurchase = (planName: string) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-purple-500/5">
      <div className="container mx-auto px-6 py-12">
        <div className="max-w-2xl mx-auto text-center mb-8">
          <Link to="/vendas" className="inline-flex items-center text-primary hover:underline mb-4">
            ...
            <Card key={index} className={`relative transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl rounded-3xl backdrop-blur-sm group ${plan.highlight ? 'ring-2 ring-primary shadow-xl scale-105' : 'hover:border-primary/30'}`}>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3 pt-4">
                  {plan.isFree ? (
                    <Button 
                      onClick={() => handlePlanSelect(plan.name, true)}
                    ...
                    <div className="space-y-2">
                      ...
                      <Button 
                        onClick={() => handleDirectPurchase(plan.name)}
                        className="w-full rounded-2xl transition-all duration-300 hover:scale-105"
                        variant="outline"
                      >
